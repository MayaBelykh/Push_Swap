...
/*   Updated: 2019/08/30 20:23:34 by ccolemon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
# define ISASCII(n) (n >= 0 && n <= 127)
# define ISDIGIT(n) (n >= '0' && n <= '9')
# define ISHALPHA(n) (n >= 'A' && n <= 'Z')
# define ISLALPHA(n) (n >= 'a' && n <= 'z')
# define ISHEX(n) (ISDIGIT(n) || ISHALPHA(n) || ISLALPHA(n))
...
** struct ft_printf.h
**
**	[%][flags][minimum field-width][precision][modif][types]
**	flags		# 0 - + 32
**	width		num *
**	precision	.num .*
**	modif		hh h l ll L z j
**	type		c s p di D o O u U xX f %
*/
...
int					p_char(char *fmt, va_list *ap, t_ap *val, int type);
int					p_str(char *fmt, va_list *ap, t_ap *val, int type);
int					p_unsigned_int(char *fmt, va_list *ap, t_ap *val, int type);
int					p_dec(char *fmt, va_list *ap, t_ap *val, int type);
int					p_hex(char *fmt, va_list *ap, t_ap *val, int type);
int					p_octal(char *fmt, va_list *ap, t_ap *val, int type);
int					p_float(char *fmt, va_list *ap, t_ap *val, int type);
...
char				*funfree(char *(*f)(const char *, const char *), char *p,
														char *p2, int check);
